...
async function updateTaskStatus(taskId, newStatus) {
  ...
  try {
    ...
    if (idx !== -1) allTasks[idx] = data.data;
    renderTasks();
  } catch (err) {
    console.error(err);
...
function createStatusButtons(taskId, currentStatus) {
  ...
  if (currentStatus === "TODO") {
    buttons.push(
      `<button class="btn btn-warning btn-sm" onclick="updateTaskStatus(${taskId}, 'IN_PROGRESS')">→ In Progress</button>`
    );
    buttons.push(
    ...
    buttons.push(
      `<button class="btn btn-secondary btn-sm" onclick="updateTaskStatus(${taskId}, 'TODO')">← To Do</button>`
    );
    buttons.push(
      `<button class="btn btn-success btn-sm" onclick="updateTaskStatus(${taskId}, 'DONE')">→ Done</button>`
    );
...
document.addEventListener("DOMContentLoaded", () => {
  console.log("🚀 หื่นจรวดพุ่ง");
  console.log("📊 Architecture: จกเห็ด");
  fetchTasks();
});
